...
describe('E2E - Serve Mode Contradiction', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('should trigger a Serve Mode alert when a contradiction is detected and survives the critic pass', async () => {
    // Arrange mocks
    ...
    (verifyContradiction as jest.Mock).mockResolvedValue({
      approved: true,
      reason: 'Opposing decisions regarding database migration'
    });
    ...
    expect(result.alertBlocks).not.toBeNull();
    expect(result.criticReasoning).toBe('Opposing decisions regarding database migration');
  });
});
