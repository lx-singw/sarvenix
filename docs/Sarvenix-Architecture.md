...
│  (Slack Bolt app · Agent runtime · Gemini as reasoning engine)      │
│                                                                       │
│   ┌───────────────┐   ┌─────────────────┐   ┌────────────────────┐ │
...
- **Normalization step:** All ingested events are passed through an entity-extraction prompt (Gemini) that tags: *decision*, *rationale*, *actor*, *artifact reference* (ticket/PR/doc ID), and *timestamp*. This is what allows a Slack sentence, a Jira comment, and a GitHub PR review to be recognized as *the same event described three ways*.
...
### 4.3 Reasoning / Synthesis Engine (Gemini)
...
4. Gemini synthesizes a single answer, citing each source inline:
...
- **Flow:** RTS query scoped to channels the user belongs to, filtered to the user's OOO time window → Gemini produces a strict 3-bullet brief:
...
- Severity tagging (🔴 needs action / 🟡 relevant / 🟢 FYI) is generated by Gemini classifying each candidate item against the user's known ownership (from the Knowledge Graph's `OWNED_BY` edges).
...
| Reasoning engine | Gemini (via Gemini API), tool-use for MCP calls |
...
2. Ask-mode synthesis with GitHub + Jira MCP (no graph yet — direct retrieval + Gemini synthesis).
